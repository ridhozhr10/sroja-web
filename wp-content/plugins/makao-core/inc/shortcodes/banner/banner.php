...
if ( ! function_exists( 'makao_core_add_banner_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function makao_core_add_banner_shortcode( $shortcodes ) {
		$shortcodes[] = 'MakaoCoreBannerShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'makao_core_filter_register_shortcodes', 'makao_core_add_banner_shortcode' );
}

if ( class_exists( 'MakaoCoreShortcode' ) ) {
	class MakaoCoreBannerShortcode extends MakaoCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'makao_core_filter_banner_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'makao_core_filter_banner_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( MAKAO_CORE_SHORTCODES_URL_PATH . '/banner' );
			$this->set_base( 'makao_core_banner' );
			$this->set_name( esc_html__( 'Banner', 'makao-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds banner element', 'makao-core' ) );
			...
			$this->set_option( array(
				...
				'title'      => esc_html__( 'Subtitle Margin Top', 'makao-core' ),
				'group'      => esc_html__( 'Content', 'makao-core' )
			) );
			$this->set_option( array(
				'field_type' => 'text',
		...
		public function render( $options, $content = null ) {
			...
			return makao_core_get_template_part( 'shortcodes/banner', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
