...
if ( ! class_exists( 'MakaoCoreShortcodes' ) ) {
	class MakaoCoreShortcodes {
		...
		function init() {
			...
			include_once MAKAO_CORE_SHORTCODES_PATH . '/dashboard/customizer/shortcodes-customizer-options.php';
		}
		...
		function set_enabled_shortcodes() {
			...
			foreach ( glob( MAKAO_CORE_SHORTCODES_PATH . '/*', GLOB_ONLYDIR ) as $shortcode ) {
				...
				if ( basename( $shortcode ) !== 'dashboard' ) {
					$is_disabled = makao_core_performance_get_option_value( $shortcode, 'makao_core_performance_shortcode_' );
		...
		function include_shortcode_classes() {
			include_once MAKAO_CORE_SHORTCODES_PATH . '/shortcode.php';
			include_once MAKAO_CORE_SHORTCODES_PATH . '/list-shortcode.php';
		}
		...
		function register_shortcodes() {
			...
			$shortcodes     = apply_filters( 'makao_core_filter_register_shortcodes', $shortcodes = array() );
	...
	MakaoCoreShortcodes::get_instance();
}