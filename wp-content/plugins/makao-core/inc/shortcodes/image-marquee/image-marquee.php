...
if ( ! function_exists( 'makao_core_add_image_marquee_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function makao_core_add_image_marquee_shortcode( $shortcodes ) {
		$shortcodes[] = 'MakaoCoreImageMarqueeShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'makao_core_filter_register_shortcodes', 'makao_core_add_image_marquee_shortcode' );
}

if ( class_exists( 'MakaoCoreShortcode' ) ) {
	class MakaoCoreImageMarqueeShortcode extends MakaoCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'makao_core_filter_image_marquee_layouts', array() ) );
			$this->set_extra_options( apply_filters( 'makao_core_filter_image_marquee_extra_options', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( MAKAO_CORE_SHORTCODES_URL_PATH . '/image-marquee' );
			$this->set_base( 'makao_core_image_marquee' );
			$this->set_name( esc_html__( 'Image Marquee', 'makao-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Image Marquee element', 'makao-core' ) );
			...
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'duration',
				'title'         => esc_html__( 'Animation Duration (Seconds)', 'makao-core' ),
				'default_value' => '20',
			) );
			$this->set_option( array(
				'field_type' => 'select',
		...
		public function render( $options, $content = null ) {
			...
			return makao_core_get_template_part( 'shortcodes/image-marquee', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
