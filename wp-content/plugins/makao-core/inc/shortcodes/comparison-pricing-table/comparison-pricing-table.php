...
if ( ! function_exists( 'makao_core_add_cpt_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function makao_core_add_cpt_shortcode( $shortcodes ) {
		$shortcodes[] = 'MakaoCoreCPTShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'makao_core_filter_register_shortcodes', 'makao_core_add_cpt_shortcode' );
}

if ( class_exists( 'MakaoCoreShortcode' ) ) {
	class MakaoCoreCPTShortcode extends MakaoCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'makao_core_filter_cpt_layouts', array() ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( MAKAO_CORE_SHORTCODES_URL_PATH . '/comparison-pricing-table' );
			$this->set_base( 'makao_core_comparison_pricing_table_holder' );
			$this->set_name( esc_html__( 'Comparison Pricing table', 'makao-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds Comparison Pricing Table holder', 'makao-core' ) );
			...
			$this->set_option( array(
				...
				'description' => esc_html__( 'Choose number of columns for features and child elements', 'makao-core' ),
				'options'     => makao_core_get_select_type_options_pool( 'columns_number', '', array( '1', '4', '5', '6' ) )
			) );
			...
			$this->set_option( array(
				'field_type'  => 'textarea',
				'name'        => 'features',
				'title'       => esc_html__( 'Features', 'makao-core' ),
				'description' => esc_html__( 'Enter features. Separate each features with new line (enter).', 'makao-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'repeater',
				'name'       => 'children',
				...
				'items'      => array(
					array(
						'field_type'    => 'text',
						'name'          => 'item_title',
						'title'         => esc_html__( 'Item Title', 'makao-core' ),
						'default_value' => ''
					),
					array(
						'field_type'  => 'text',
						'name'        => 'highlight_positions',
						'title'       => esc_html__( 'Positions of Highlight Words', 'makao-core' ),
						'description' => esc_html__( 'Enter the positions of the words that you would like to be colored. Separate the positions with commas (e.g. if you would like the first, third, and fourth word to have different color, you would enter "1,3,4")', 'makao-core' ),
						'group'       => esc_html__( 'Title Style', 'makao-core' ),
					),
					array(
						'field_type' => 'color',
						'name'       => 'highlight_color',
						'title'      => esc_html__( 'Title Highlight Color', 'makao-core' ),
						'group'      => esc_html__( 'Title Style', 'makao-core' ),
					),
					array(
						'field_type'    => 'select',
		...
		public function render( $options, $content = null ) {
			...
			return makao_core_get_template_part( 'shortcodes/comparison-pricing-table', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
