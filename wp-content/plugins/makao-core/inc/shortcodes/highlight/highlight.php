...
if ( ! function_exists( 'makao_core_add_highlight_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function makao_core_add_highlight_shortcode( $shortcodes ) {
		$shortcodes[] = 'MakaoCoreHighlightShortcode';

		return $shortcodes;
	}

	add_filter( 'makao_core_filter_register_shortcodes', 'makao_core_add_highlight_shortcode' );
}

if ( class_exists( 'MakaoCoreShortcode' ) ) {
	class MakaoCoreHighlightShortcode extends MakaoCoreShortcode {

		public function map_shortcode() {
			$this->set_shortcode_path( MAKAO_CORE_SHORTCODES_URL_PATH . '/highlight' );
			$this->set_base( 'makao_core_highlight' );
			$this->set_name( esc_html__( 'Highlight', 'makao-core' ) );
			$this->set_description( esc_html__( 'Shortcode that displays highlight with provided parameters', 'makao-core' ) );
			...
			$this->set_option( array(
				...
				'title'         => esc_html__( 'Disable Text Line Break', 'makao-core' ),
				'description'   => esc_html__( 'Enabling this option will disable text line breaks for screen size 1024 and lower', 'makao-core' ),
			...
			$this->set_option( array(
				...
				'title'       => esc_html__( 'Highlight Text Position', 'makao-core' ),
				'description' => esc_html__( 'Enter the positions of the words you would like to display as "highlight" text. Separate start and end positions with commas (e.g. if you would like to wrap from fifth to ninth words, you would enter "5,9") or if you want to highlight whole text fill -1', 'makao-core' ),
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_color',
				'title'      => esc_html__( 'Highlight Text Color', 'makao-core' )
			) );
			$this->set_option( array(
				'field_type' => 'color',
				'name'       => 'highlight_background_color',
				'title'      => esc_html__( 'Highlight Text Background Color', 'makao-core' )
			) );
			$this->set_option( array(
				'field_type'    => 'text',
				'name'          => 'link',
		...
		public function render( $options, $content = null ) {
			...
			return makao_core_get_template_part( 'shortcodes/highlight', 'templates/highlight', '', $atts );
		}
