...
if ( ! function_exists( 'makao_core_add_icon_with_text_shortcode' ) ) {
	/**
	 ...
	 * @return array
	 */
	function makao_core_add_icon_with_text_shortcode( $shortcodes ) {
		$shortcodes[] = 'MakaoCoreIconWithTextShortcode';
		
		return $shortcodes;
	}
	
	add_filter( 'makao_core_filter_register_shortcodes', 'makao_core_add_icon_with_text_shortcode' );
}

if ( class_exists( 'MakaoCoreShortcode' ) ) {
	class MakaoCoreIconWithTextShortcode extends MakaoCoreShortcode {
		
		public function __construct() {
			$this->set_layouts( apply_filters( 'makao_core_filter_icon_with_text_layouts', array() ) );
			
			$options_map = makao_core_get_variations_options_map( $this->get_layouts() );
			$default_value = $options_map['default_value'];
			
			$this->set_extra_options( apply_filters( 'makao_core_filter_icon_with_text_extra_options', array(), $default_value ) );
			
			parent::__construct();
		}
		
		public function map_shortcode() {
			$this->set_shortcode_path( MAKAO_CORE_SHORTCODES_URL_PATH . '/icon-with-text' );
			$this->set_base( 'makao_core_icon_with_text' );
			$this->set_name( esc_html__( 'Icon With Text', 'makao-core' ) );
			$this->set_description( esc_html__( 'Shortcode that adds icon with text element', 'makao-core' ) );
		...
		public function render( $options, $content = null ) {
			...
			return makao_core_get_template_part( 'shortcodes/icon-with-text', 'variations/' . $atts['layout'] . '/templates/' . $atts['layout'], '', $atts );
		}
		...
		private function get_holder_classes( $atts ) {
			...
			$holder_classes = apply_filters( 'makao_core_filter_icon_with_text_variation_classes', $holder_classes, $atts );
