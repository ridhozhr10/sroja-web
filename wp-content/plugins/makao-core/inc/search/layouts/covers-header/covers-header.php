<?php
class CoversHeaderSearch extends MakaoCoreSearch {
	...
	public function return_template(){
		return makao_core_template_part( 'search/layouts/' . $this->search_layout, 'templates/' . $this->search_layout );
	}
	...
	public function load_template() {
		if ( is_active_widget( false, false, 'makao_core_search_opener' ) ) {
			...
			$actions         = array();
			$sidebars        = qode_framework_get_widget_sidebars( 'makao_core_search_opener' );
			...
			foreach ( $sidebars as $sidebar ) {
				if ( $sidebar == 'qodef-header-widget-area-one' || $sidebar == 'qodef-header-widget-area-two' ) {
					$actions[] = 'makao_action_after_page_header_inner';
					$actions[] = 'makao_core_action_after_sticky_header';
				} elseif ( $sidebar == 'qodef-mobile-header-widget-area' ) {
					$actions[] = 'makao_action_after_page_mobile_header_inner';
				} elseif ( $sidebar == 'qodef-top-area-left' || $sidebar == 'qodef-top-area-right' ) {
					$actions[] = 'makao_core_action_after_top_area';
