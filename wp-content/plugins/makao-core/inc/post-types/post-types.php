...
if ( ! class_exists( 'MakaoCoreCustomPostTypes' ) ) {
	class MakaoCoreCustomPostTypes {
		...
		function init() {
			...
			include_once MAKAO_CORE_CPT_PATH . '/dashboard/customizer/post-types-customizer-options.php';
		}
		...
		function set_enabled_post_types() {
			...
			foreach ( glob( MAKAO_CORE_CPT_PATH . '/*', GLOB_ONLYDIR ) as $post_type ) {
				...
				if ( basename( $post_type ) !== 'dashboard' ) {
					$is_disabled = makao_core_performance_get_option_value( $post_type, 'makao_core_performance_post_type_' );
			...
			do_action( 'makao_core_action_add_custom_post_type', $this );
		}
		...
		function register_post_types() {
			...
			$cpts           = apply_filters( 'makao_core_filter_register_custom_post_types', $cpts = array() );
		...
		function include_taxonomies() {
			...
			do_action( 'makao_core_action_include_cpt_tax_fields' );
		}
		
		function register_taxonomies() {
			// Hook to includes custom post types taxonomy fields
			do_action( 'makao_core_action_register_cpt_tax_fields' );
		}
	...
	MakaoCoreCustomPostTypes::get_instance();
}