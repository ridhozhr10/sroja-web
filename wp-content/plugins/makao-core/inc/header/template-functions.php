...
if ( ! function_exists( 'makao_core_get_header_logo_image' ) ) {
	/**
	 * This function print header logo image
	 *
	 * @param $parameters array
	 */
	function makao_core_get_header_logo_image( $parameters = array() ) {
		...
		$parameters = array_merge( $parameters, array(
			...
			'logo_height'  => ! empty( $logo_height ) && empty($parameters['sticky_logo'])  ? 'height:' . intval( $logo_height ) . 'px' : '',
		) );
		...
		foreach ( $available_logos as $logo ) {
			...
			$logo_image_id = makao_core_get_post_value_through_levels( 'qodef_logo_' . $logo );
			
			if ( empty( $logo_image_id ) && ! empty( $header_skin ) ) {
				$logo_image_id = makao_core_get_post_value_through_levels( 'qodef_logo_main' );
			}
			...
			if ( ! empty( $logo_image_id ) ) {
				$logo_image_attr = array(
					...
					'itemprop' => 'image',
					'alt'      => sprintf( esc_attr__( 'logo %s', 'makao-core' ), $logo )
				);
		...
		if ( $is_enabled ) {
			echo apply_filters( 'makao_core_filter_get_header_logo_image', makao_core_get_template_part( 'header/templates', 'parts/logo', '', $parameters ), $parameters );
...
if ( ! function_exists( 'makao_core_get_header_widget_area' ) ) {
	/**
	 ...
	 */
	function makao_core_get_header_widget_area( $header_layout = '', $widget_area = 'one' ) {
		$page_id = qode_framework_get_page_id();

		$widget_area_map = apply_filters( 'makao_core_filter_header_widget_area', array(
			'page_id'             => $page_id,
