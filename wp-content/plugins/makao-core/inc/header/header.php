...
abstract class MakaoCoreHeader {
	...
	public function __construct() {
		...
		add_filter( 'makao_core_action_before_main_css', array( $this, 'enqueue_assets' ) );
		add_filter( 'makao_core_action_before_main_css', array( $this, 'enqueue_additional_assets' ) );
		...
		add_filter( 'makao_filter_add_inline_style', array( $this, 'set_inline_header_styles' ) );
		add_filter( 'makao_filter_header_inner_class', array( $this, 'set_grid_class' ) );
		add_filter( 'makao_core_filter_nav_menu_header_selector', array( $this, 'set_nav_menu_header_selector' ) );
		add_filter( 'makao_core_filter_nav_menu_narrow_header_selector', array( $this, 'set_nav_menu_narrow_header_selector' ) );
	}
	...
	public function load_template( $parameters = array() ) {
		$parameters = apply_filters( 'makao_core_filter_header_template', $parameters );
		
		return makao_core_get_template_part( 'header/layouts/' . $this->slug, 'templates/' . $this->slug, '', $parameters );
	}
	...
	public function get_header_transparency() {
		$background_color = makao_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_slug() . '_header_background_color' );
	...
	function set_header_height() {
		$header_height = makao_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_slug() . '_header_height' );
		$header_height = ! empty( $header_height ) ? intval( $header_height ) : $this->default_header_height;
		
		$this->header_height = apply_filters( 'makao_core_filter_set_header_height', $header_height );
	}
	
	function set_grid_class( $class ) {
		$class .= makao_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_slug() . '_header_in_grid' ) == 'yes' ? 'qodef-content-grid' : '';
	...
	public function set_inline_header_styles( $style ) {
		...
		$height           = makao_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_slug() . '_header_height' );
		$background_color = makao_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_slug() . '_header_background_color' );
		...
		$side_padding = makao_core_get_post_value_through_levels( 'qodef_' . $this->get_formatted_slug() . '_header_side_padding' );
