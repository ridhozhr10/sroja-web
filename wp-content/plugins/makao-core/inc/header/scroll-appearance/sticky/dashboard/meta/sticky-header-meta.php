...
if ( ! function_exists( 'makao_core_add_sticky_header_meta_options' ) ) {
	function makao_core_add_sticky_header_meta_options( $section, $custom_sidebars ) {
		...
		if ( $section ) {
			...
			$section->add_field_element(
				array(
					...
					'title'       => esc_html__( 'Choose Custom Sticky Header Widget Area', 'makao-core' ),
					'description' => esc_html__( 'Choose custom widget area to display in sticky header widget area', 'makao-core' ),
	...
	add_action( 'makao_core_action_after_header_scroll_appearance_meta_options_map', 'makao_core_add_sticky_header_meta_options', 10, 2 );
}

if ( ! function_exists( 'makao_core_add_sticky_header_logo_meta_options' ) ) {
	function makao_core_add_sticky_header_logo_meta_options( $logo_tab, $header_logo_section ) {
	...
	add_action( 'makao_core_action_after_page_logo_meta_map', 'makao_core_add_sticky_header_logo_meta_options', 10, 2 );
}