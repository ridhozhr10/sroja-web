...
class MakaoCoreTitles {
	...
	public function __construct() {
		...
		add_filter( 'makao_filter_page_title_classes', array( $this, 'add_page_title_classes' ) );
		
		// Add title area inline styles
		add_filter( 'makao_filter_add_inline_style', array( $this, 'add_inline_styles' ) );
	}
	...
	function include_layouts() {
		...
		foreach ( glob( MAKAO_CORE_INC_PATH . '/title/dashboard/*/*.php' ) as $admin ) {
			include_once $admin;
		}
		
		foreach ( glob( MAKAO_CORE_INC_PATH . '/title/layouts/*/include.php' ) as $layout ) {
			include_once $layout;
		}
	}
	
	function set_variables() {
		$layout_meta = makao_core_get_post_value_through_levels( 'qodef_title_layout' );
		$layouts     = apply_filters( 'makao_core_filter_register_title_layouts', $layouts = array() );
	...
	function render_template() {
		...
		if ( ! empty( $title_object ) ) {
			$template_hook = $title_object->overriding_whole_title ? 'makao_filter_title_template' : 'makao_filter_title_content_template';
	...
	function add_page_title_classes( $classes ) {
		$layout             = makao_core_get_post_value_through_levels( 'qodef_title_layout' );
		$text_alignment     = makao_core_get_post_value_through_levels( 'qodef_page_title_text_alignment' );
		$vertical_alignment = makao_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$image_params       = makao_core_get_page_title_image_params();
	...
	function add_inline_styles( $style ) {
		...
		$height           	  	  = makao_core_get_post_value_through_levels( 'qodef_page_title_height' );
		$height_mobile     	  	  = makao_core_get_post_value_through_levels( 'qodef_page_title_height_on_smaller_screens' );
		$title_padding    	  	  = apply_filters( 'makao_core_filter_title_padding', 0 );
		$title_padding_mobile 	  = apply_filters( 'makao_core_filter_title_padding_mobile', 0 );
		$title_vertical_alignment = makao_core_get_post_value_through_levels( 'qodef_page_title_vertical_text_alignment' );
		$background_color 	  	  = makao_core_get_post_value_through_levels( 'qodef_page_title_background_color' );
		$image_params     	  	  = makao_core_get_page_title_image_params();
        $title_side_padding 	  = makao_core_get_post_value_through_levels( 'qodef_page_title_side_padding' );
        ...
        $full_width_content_styles = array();

        if ( ! empty( $title_side_padding ) ) {
            $full_width_content_styles['padding-left'] =  $title_side_padding . '!important';
            $full_width_content_styles['padding-right'] =  $title_side_padding . '!important';
        }

        if ( ! empty( $full_width_content_styles ) ) {
            $style .= qode_framework_dynamic_style( '.qodef-page-title .qodef-m-content.qodef-content-full-width', $full_width_content_styles );
        }
		
		$title_styles = array();
		
		$title_color = makao_core_get_post_value_through_levels( 'qodef_page_title_color' );
