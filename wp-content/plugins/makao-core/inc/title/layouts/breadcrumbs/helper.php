...
if ( ! function_exists( 'makao_core_register_breadcrumbs_title_layout' ) ) {
	function makao_core_register_breadcrumbs_title_layout( $layouts ) {
		$layouts['breadcrumbs'] = 'MakaoCoreBreadcrumbsTitle';
		
		return $layouts;
	}
	
	add_filter( 'makao_core_filter_register_title_layouts', 'makao_core_register_breadcrumbs_title_layout');
}

if ( ! function_exists( 'makao_core_add_breadcrumbs_title_layout_option' ) ) {
	/**
	 ...
	 * @return array
	 */
	function makao_core_add_breadcrumbs_title_layout_option( $layouts ) {
		$layouts['breadcrumbs'] = esc_html__( 'Breadcrumbs', 'makao-core' );
		
		return $layouts;
	}
	
	add_filter( 'makao_core_filter_title_layout_options', 'makao_core_add_breadcrumbs_title_layout_option' );
}

if ( ! function_exists( 'makao_core_breadcrumbs' ) ) {
	/**
	 * Function that renders breadcrumbs html
	 */
	function makao_core_breadcrumbs() {
		$page_id = qode_framework_get_page_id();
		
		// Breadcrumbs label
		$labels = apply_filters( 'makao_core_filter_breadcrumbs_label', array(
			'home'        => esc_html__( 'Home', 'makao-core' ),
			'tag'         => esc_html__( 'Posts tagged "%s"', 'makao-core' ),
			'author'      => esc_html__( 'Posted by %s', 'makao-core' ),
			'search'      => esc_html__( 'Search results for "%s"', 'makao-core' ),
			'404'         => esc_html__( '404 - Page not found', 'makao-core' ),
			'query_paged' => esc_html__( '(Page %s)', 'makao-core' )
		) );
		
		// Breadcrumbs variables
		$settings = apply_filters( 'makao_core_filter_breadcrumbs_settings', array(
		...
		$breadcrumbs_html = '';
		if ( ! empty( $wrap ) ) {
			$breadcrumbs_html = $settings['wrap_before'] . $wrap . apply_filters( 'makao_core_filter_breadcrumbs_content', $wrap_child, $settings ) . $settings['wrap_after'];
		}
		
		echo apply_filters( 'makao_core_filter_breadcrumbs_template', $breadcrumbs_html );
	}
}