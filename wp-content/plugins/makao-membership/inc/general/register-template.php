...
if ( ! class_exists( 'MakaoMembershipPageTemplates' ) ) {
	class MakaoMembershipPageTemplates {
		...
		function __construct() {
			...
			$this->set_templates( array(
				'user-dashboard.php' => esc_html__( 'User Dashboard', 'makao-membership' )
			) );
			...
			add_filter( 'makao_membership_filter_dashboard_template_name', array( $this, 'set_dashboard_template_name' ) );
			
			// Include dashboard page template content when user is logged in
			add_action( 'makao_membership_action_after_user_dashboard_page_content', array( $this, 'include_dashboard_template_content' ) );
		}
		...
		public function get_template_path( $template ) {
			...
			if ( isset( $post ) && ! empty( $post ) ) {
				...
				$file = MAKAO_MEMBERSHIP_INC_PATH . '/general/page-templates/' . $page_template;
		...
		function include_dashboard_template_content() {
			...
			if ( is_user_logged_in() ) {
				makao_membership_template_part( 'general', 'page-templates/parts/content' );
			}
		}
	}
	
	MakaoMembershipPageTemplates::get_instance();
}