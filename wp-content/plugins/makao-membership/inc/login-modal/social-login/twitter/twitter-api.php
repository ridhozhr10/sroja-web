...
if ( ! class_exists( 'MakaoMembershipTwitterApi' ) ) {
	/**
	 * Class MakaoMembershipTwitterApi
	 */
	class MakaoMembershipTwitterApi {
		/**
		 * @var $instance MakaoMembershipTwitterApi class object
		 */
		...
		/**
		 * @return MakaoMembershipTwitterApi
		 */
		 ...
		 *
		 * @see MakaoMembershipTwitterApi::generateSignatureBase()
		 */
		...
		public function obtainRequestToken() {
			...
			if ( is_wp_error( $response ) ) {
				...
				$responseObj->message = esc_html__( 'Internal WP error', 'makao-membership' );
			} else {
				...
				if ( ! empty( $responseBody ) ) {
					...
					if ( ( is_array( $responseParsed ) && count( $responseParsed ) ) && ! empty( $responseParsed['oauth_token'] ) && ! empty( $responseParsed['oauth_token_secret'] ) ) {
						...
						if ( empty( $responseObj->redirectUrl ) ) {
							...
							$responseObj->message                  = esc_html__( 'Redirect URL could not be generated', 'makao-membership' );
						} else {
							$responseObj->oauth_callback_confirmed = true;
						}
					} else {
						$responseObj->oauth_callback_confirmed = false;
						$responseObj->message                  = esc_html__( 'Couldn\'t connect with Twitter API', 'makao-membership' );
					}
				}
			}
		...
		public function buildCurrentPageURI() {
			...
			return makao_membership_get_membership_redirect_url();
		}
	}
}