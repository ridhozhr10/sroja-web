(function ($) {
	...
	var qodefLoginModal = {
		...
		triggerShowModal: function ($holder) {
			...
			$(document.body).on('makao_membership_trigger_login_modal', function () {
		...
		triggerFormSocialSubmit: function ($holder) {
			...
			if ($form.length) {
				...
				$socialButton.find('.qodef-m-social-login-btn').on('click', function (e) {
					...
					$(document).trigger('makao_membership_social_login_is_triggered', [qodefLoginModal, $form, $(this).data('social')]);
				});
			}
		},
		triggerRequest: function ($holder, socialNetwork, socialResponse) {
			...
			var ajaxData = {
				...
				nonce: $holder.find('#makao-membership-ajax-' + $requestType + '-nonce').val()
			};
		...
		triggerForceLogin: function ($holder, ajaxData, redirect) {
			...
			ajaxData.nonce = $holder.parent().find('#makao-membership-ajax-login-nonce').val();
	...
	var qodefFacebookLogin = {
		init: function () {
			...
			if (qodefFacebookLogin.fbIsAppIdSet()) {
				qodefFacebookLogin.fbAsyncInit(makaoMembershipGlobal.facebookAppId);
			}
		},
		fbIsAppIdSet: function () {
			return typeof makaoMembershipGlobal.facebookAppId !== 'undefined' && makaoMembershipGlobal.facebookAppId !== '';
		},
