...
(function ($) {
    ...
    $(document).on('makao_trigger_get_new_posts', function (e, holder) {
	...
	var qodefFilter = {
		...
		setVisibility: function (holder, item, triggerEvent) {
			...
			if (!isShowAllFilter && !listHasVisibleItems && qodefFilter.hasLoadMore(holder)) {
				qodef.body.trigger('makao_trigger_load_more', [holder]);
			} else {
...
(function ($) {
	...
	$(document).on('makao_trigger_load_more', function (e, $holder, nextPage) {
				...
				success: function (response) {
					...
					if (response.status === 'success') {
						...
						$itemsHolder.waitForImages(function () {
							...
							qodef.body.trigger('makao_trigger_get_new_posts', [$holder, response.data, nextPage]);
						});
						if ($holder.hasClass('qodef-pagination-type--standard')) {
							$('html, body').animate({
...
(function ($) {
	...
	//qodefCore.qodefWooMagnificPopup = qodefWooMagnificPopup;
	...
	function qodefAddButtonEl() {
		var $productList = $('.qodef-woo-product-list:not(.qodef-woo-shortcode)');
		var $listItem = $productList.find('.product');

		if ($listItem.length) {
			$listItem.each(function () {
				var $thisItem = $(this),
					$elemHolder = $thisItem.find('.qodef-woo-product-button-holder'),
