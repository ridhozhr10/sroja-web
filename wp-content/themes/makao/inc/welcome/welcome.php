...
if ( ! class_exists( 'MakaoWelcomePage' ) ) {
	class MakaoWelcomePage {
		private static $instance;

		/**
		 * get the instance of MakaoWelcomePage
		 *
		 * @return self
		 */
		...
		function init_activation_hook() {
			if ( ! is_network_admin() ) {
				set_transient( '_makao_welcome_page_redirect', 1, 30 );
			}
		}
		...
		function welcome_page_redirect() {
			...
			if ( ! get_transient( '_makao_welcome_page_redirect' ) ) {
				return;
			}

			// delete the redirect transient
			delete_transient( '_makao_welcome_page_redirect' );
			...
			wp_safe_redirect( add_query_arg( array( 'page' => 'makao_welcome_page' ), esc_url( admin_url( 'themes.php' ) ) ) );
			exit;
		}
		...
		function create_welcome_page() {
			...
			remove_submenu_page( 'themes.php', 'makao_welcome_page' );
		}
		...
		function welcome_page_content() {
			...
			$params['theme_screenshot']  = file_exists( MAKAO_ROOT_DIR . '/screenshot.png' ) ? MAKAO_ROOT . '/screenshot.png' : MAKAO_ROOT . '/screenshot.jpg';

			makao_template_part( 'welcome', 'templates/welcome', '', $params );
		}
		...
		function enqueue_styles( $hook ) {
			if ( $hook === 'appearance_page_makao_welcome_page' ) {
				wp_enqueue_style( 'makao-welcome-page-style', MAKAO_INC_ROOT . '/welcome/assets/admin/css/welcome.min.css' );
			}
		}
	}
}

MakaoWelcomePage::get_instance();